...
/** Shared runtime dependencies exposed only to a host extension during startup. */
export interface ConnectorRuntimeExtensionContext extends Pick<
  ConnectAppOptions,
  "actionPolicy" | "adminToken" | "logger" | "runtimeDatabase" | "secretCodec"
> {
  dataDir: string;
}

/** Optional route and action-runner hooks returned by a host extension. */
export interface ConnectorRuntimeExtension {
  registerRoutes?: ConnectAppOptions["registerStaticRoutes"];
  wrapActionRunner?: ConnectAppOptions["wrapActionRunner"];
}
...
export interface ConnectorRuntimeOptions {
  ...
  /** Install a host extension after storage opens and before the request handler is built. */
  extend?: (context: ConnectorRuntimeExtensionContext) => Promise<ConnectorRuntimeExtension>; // lens-seam
}
...
async function openRuntime(options: ConnectorRuntimeOptions): Promise<ConnectorRuntime> {
  ...
  let closeFiles = (): void => {};
  try {
    const actionPolicy = new ActionPolicyService(options.actionPolicy);
    const extension = await options.extend?.({
      actionPolicy,
    ...
    const { app, runtimeAuthConfigured } = await createConnectApp({
      ...
      registerStaticRoutes: extension?.registerRoutes, // lens-seam
      wrapActionRunner: extension?.wrapActionRunner, // lens-seam
      logger: options.logger,
